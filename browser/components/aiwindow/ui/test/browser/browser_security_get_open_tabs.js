...
/**
 * Pin strictly increasing lastAccessed timestamps onto the given tabs, in
 * array order, so the get_open_tabs sort order is deterministic. tab.lastAccessed
 * returns Date.now() for the selected tab of each window (its backing
 * _lastAccessed is Infinity), so the last-opened tab - and, across windows,
 * every window's selected tab - would otherwise tie on Date.now() and sort
 * nondeterministically. Writing a finite _lastAccessed makes the getter return
 * exactly that value; updateLastAccessed() can't be used because it re-forces
 * Infinity for the selected tab. The earliest tab in the array gets the oldest
 * timestamp, matching the order serveSharedHTMLInTab opens foreground tabs.
 *
 * @param {MozTabbrowserTab[]} tabsOldestFirst
 */
function pinLastAccessed(tabsOldestFirst) {
  const base = Date.now() - tabsOldestFirst.length * 1000;
  tabsOldestFirst.forEach((tab, index) => {
    tab._lastAccessed = base + index * 1000;
  });
}
...
add_task(async function test_get_open_tabs_returns_sanitized_data() {
  ...
  // lastAccessed is pinned explicitly below (see pinLastAccessed) so the
  // open order maps to openOrder[0] < openOrder[1] < openOrder[2]. After
  // sort-by-lastAccessed-desc, get_open_tabs returns the reverse.
  ...
  // Pin after the about tab opens: that selection deselects httpTab3 and
  // rewrites its _lastAccessed, so pinning earlier would be clobbered.
...
add_task(async function test_get_open_tabs_aggregates_across_ai_windows() {
  ...
  // Pinned newer than tabInWinA below, so it should sort first by lastAccessed.
  ...
  // Each window keeps its own selected tab, so tabInWinA and tabInWinB are
  // both Infinity (-> Date.now()) and would tie without explicit pinning.
